...
def addartist():
    ...
    cursor.execute("""INSERT INTO Artists (name, address, town, county, postcode) VALUES (?,?,?,?,?)""",
                   (newname, newaddress, newtown, newcounty, newpostcode))
    db.commit()
...
def addart():
    ...
    cursor.execute("""INSERT INTO Art (artistid, title, medium, price) VALUES (?,?,?,?)""",
                   (newartname, newtitle, newmedium, newprice))
    db.commit()
...
def viewartists():
    ...
    for x in cursor.fetchall():
        newrecord = f"{x[0]},{x[1]},{x[2]},{x[3]},{x[4]}\n"
...
def searchmediumoutput():
    ...
    cursor.execute("""SELECT Art.pieceid, Artists.name, Art.title, Art.medium, Art.price 
                      FROM Artists, Art 
                      WHERE Artists.artistid=Art.artistid AND Art.medium=?""", (selectedmedium,))
    for x in cursor.fetchall():
        newrecord = f"{x[0]}, {x[1]}, {x[2]}, {x[3]}, £{x[4]}\n"
        outputwindow.insert(END, newrecord)
    medium2.set("")
...
def searchbyprice():
    ...
    cursor.execute("""SELECT Art.pieceid, Artists.name, Art.title, Art.medium, Art.price 
                      FROM Artists, Art 
                      WHERE Artists.artistid=Art.artistid AND Art.price>=? AND Art.price<=?""", [minprice, maxprice])
    for x in cursor.fetchall():
        newrecord = f"{x[0]}, {x[1]}, {x[2]}, {x[3]}, £{x[4]}\n"
        outputwindow.insert(END, newrecord)
...
clearartbtn = Button(text="Clear Piece", command=clearartist)
